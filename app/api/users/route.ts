...
import { forbidden, getRequestUser, hashPassword, isTrustedMutationRequest, normalizeUsername, PASSWORD_MAX_LENGTH, PASSWORD_MIN_LENGTH, unauthorized, untrustedRequest, validPassword, validUsername } from "@/lib/auth";
...
export async function POST(request: NextRequest) {
  ...
  if (payload.action === "reset_password") {
    const id = Number(payload.id);
    const password = String(payload.password ?? "");
    if (!Number.isInteger(id) || !validPassword(password)) return Response.json({ error: `Informe uma senha entre ${PASSWORD_MIN_LENGTH} e ${PASSWORD_MAX_LENGTH} caracteres.` }, { status: 400 });
